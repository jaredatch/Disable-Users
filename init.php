...
final class ja_disable_users {
	...
	public function my_bulk_action_handler( $redirect_to, $doaction, $userIDs ) {
		if ( $doaction !== 'toggle_user' ) {
			return $redirect_to;
		}
		...
		if ( current_user_can( $this->get_edit_cap() ) ) {
			...
			foreach ( $userIDs as $userID ) {
				...
				if ( is_multisite() && is_super_admin( (int) $userID ) ) {
					wp_die( __( 'Super admins can not be disabled.', 'ja_disable_users' ) );
				}

				$user_disabled = get_the_author_meta( 'ja_disable_user', $userID );
				if($user_disabled) {
					$nonce_name =  'ja_enable_user_' ;
				} else {
					$nonce_name =  'ja_disable_user_' ;
				}

				update_user_meta(  (int) $userID, 'ja_disable_user', ( $nonce_name === 'ja_disable_user_' ? true : false ) );
	...
	public function register_my_bulk_actions($bulk_actions) {
	  $bulk_actions['toggle_user'] = __( 'Toggle User Permission', 'toggle_user');
	  return $bulk_actions;
	}
